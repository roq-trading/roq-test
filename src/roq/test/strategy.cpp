...
Strategy::Strategy(client::Dispatcher &dispatcher)
    : _dispatcher(dispatcher),
      _depth_builder(client::DepthBuilderFactory::create("test", _depth)),
...
void Strategy::operator()(const Event<Timer> &event) {
  if (_stop) _dispatcher.stop();
  if (_state) (*_state)(event.value.now);
}
...
void Strategy::operator()(const Event<DownloadEnd> &event) {
  ...
  if (event.value.account.empty()) {
    ...
    _order_id = std::max(_order_id, event.value.max_order_id);
  }
  check_ready();
}
...
void Strategy::operator()(const Event<ReferenceData> &event) {
  ...
  update(_reference_data.tick_size, event.value.tick_size);
  update(_reference_data.min_trade_vol, event.value.min_trade_vol);
  check_ready();
}
...
void Strategy::operator()(const Event<MarketByPriceUpdate> &event) {
  _depth_builder->update(event.value);
  VLOG(1)(R"(depth=[{}])", fmt::join(_depth, R"(, )"));
  check_depth();
}
...
void Strategy::operator()(const Event<OrderUpdate> &event) {
  ...
  if (static_cast<bool>(_state) == true) (*_state)(event.value);
}
...
void Strategy::check_depth() {
  auto ready = _ready && std::fabs(_depth[0].bid_quantity) > 0.0 &&
...
void Strategy::check_ready() {
  auto ready = _market_data.download == false && _market_data.ready == true &&
               _order_manager.download == false &&
               _order_manager.ready == true &&
               std::fabs(_reference_data.tick_size) > 0.0 &&
               std::fabs(_reference_data.min_trade_vol) > 0.0 &&
               _reference_data.trading == true;
  if (update(_ready, ready) && _ready) LOG(INFO)("*** INSTRUMENT READY ***");
}
