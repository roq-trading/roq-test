...
void ModifyOrderState::operator()(const OrderAck &order_ack) {
  LOG_IF(FATAL, order_ack.type != RequestType::MODIFY_ORDER)("Unexpected"sv);
  switch (order_ack.origin) {
    case Origin::GATEWAY:
      switch (order_ack.status) {
        case RequestStatus::FORWARDED:
          LOG_IF(FATAL, exchange_ack_)("Unexpected"sv);
          gateway_ack_ = true;
          break;
        default:
    ...
    case Origin::EXCHANGE:
      switch (order_ack.status) {
        case RequestStatus::ACCEPTED:
          LOG_IF(FATAL, gateway_ack_ == false)("Unexpected request status"sv);
          exchange_ack_ = true;
          break;
        default:
          LOG(FATAL)("Unexpected request status"sv);
